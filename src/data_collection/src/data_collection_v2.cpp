...
int main(int argc, char** argv){
    ...
    int size_lidar;
    std::string image_topic, lidar_topic, save_path;
    nh.getParam("image_topic", image_topic);
    nh.getParam("lidar_topic", lidar_topic);
    nh.getParam("save_path", save_path);
    nh.getParam("size_lidar", size_lidar);

    uint64 ros_time = ros::Time::now().toNSec();
    std::stringstream filename;
    filename << ros_time;
    std::string NAME = filename.str();
    std::string LIDAR_DIR = save_path + NAME + ".pcd";
    std::string IMAGE_DIR = save_path + NAME + ".png";
    ROS_INFO_STREAM(LIDAR_DIR);
    ...
    while (ros::ok())
    {
        if(cloud.size() >= size_lidar){
            ROS_INFO_STREAM("finish collecting lidar data.");
            break;
        }
    }
    ...
    if (cloud.size()<size_lidar)
    {
        ROS_INFO_STREAM("data collection failed!");
        ros::shutdown();
    }
    else
    {
        ...
        fused_cloud.width = 1;
        //
        pcl::io::savePCDFile(LIDAR_DIR, fused_cloud);
        imwrite(IMAGE_DIR, color_img);
        ROS_INFO_STREAM("data collection finished!");
        ros::waitForShutdown();
    }
    ros::waitForShutdown();
}